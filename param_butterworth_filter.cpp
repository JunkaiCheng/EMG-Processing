...
CParamButterworthFilter::CParamButterworthFilter(): CParam("CParamButterworthFilter")
{
	ccof.clear();
	dcof.clear();
}

CParamButterworthFilter::~CParamButterworthFilter()
{
}

CParamButterworthFilter::CParamButterworthFilter(const CParamButterworthFilter& rhs): CParam(rhs)
{
	CParamButterworthFilter::operator=(rhs);
}

CParamButterworthFilter::CParamButterworthFilter(const std::string& class_name): CParam(class_name)
{
	ccof.clear();
	dcof.clear();
}

CParamButterworthFilter& CParamButterworthFilter::operator = (const CParamButterworthFilter& rhs)
{
	CParam::operator=(rhs);
	ccof = rhs.ccof;
	dcof = rhs.dcof;
...
CObject* CParamButterworthFilter::clone() const
{
	return new CParamButterworthFilter(*this);
}

std::shared_ptr<CObject> CParamButterworthFilter::clone_shared() const
{
	return std::make_shared<CParamButterworthFilter>(*this);
}

bool CParamButterworthFilter::isSameKindAs(const CObject* ptr)
{
	return dynamic_cast<const CParamButterworthFilter*>(ptr) != nullptr;
}

bool CParamButterworthFilter::isSameKindAs(const std::shared_ptr<CObject> ptr)
{
	return isSameKindAs(ptr.get());
}

void CParamButterworthFilter::setParams(string filter_type, int filter_order, bool scale_flag, float f1f, string filename_coef, float f2f)
{
	// ---------------------------------------------------------------------------------------------------
	// 检查输入
	// ---------------------------------------------------------------------------------------------------
	if (!(filter_type == "lowpass" || filter_type == "highpass" || filter_type == "bandpass" || filter_type == "bandstop")) {
		cout << "filter supports types: lowpass highpass bandpass bandstop, while your input filter type " << filter_type << "is not allowed." << endl;
		assert(false);
	}
	if ((filter_type == "bandpass" || filter_type == "bandstop")) {
		if (!(f1f < f2f)) {
			cout << "Bandpass and Bandstop should make f2f: " << f2f << " strictly bigger than f1f: " << f1f << "." << endl;
			assert(false);
		}
	}
	if (!(filter_order >= 1 && filter_order <= 100)) {
		cout << "filter order should be greater or equal to 1 and smaller or equal to 100, while your input order: " << filter_order << " is not allowed." << endl;
		assert(false);
	}
	if (!(f1f > 0 && f1f < 1)) {
		cout << "f1f should be strictly greater than 0 and strictly smaller than 1, while your input f1f " << f1f << " is not allowed." << endl;
		cout << "If fs is the frequency at which the data is sampled (samples per second) and f is the cutoff frequency you want, " << endl;
		cout << "then the parameter you would input to the program is 2*f/fs. " << endl;
		cout << "For example if your data is sampled at 5000 samples/sec and you want a bandpass filter from 500 to 1000 Hz, " << endl;
		cout << "then the lower cutoff would be 2*500/5000=0.2 and the upper cutoff would be 2*1000/5000=0.4." << endl;
		assert(false);
	}
	if ((filter_type == "bandpass" || filter_type == "bandstop")) {
		if (!(f2f > 0 && f2f < 1)) {
			cout << "f2f should be strictly greater than 0 and strictly smaller than 1, while your input f2f " << f2f << " is not allowed." << endl;
			cout << "If fs is the frequency at which the data is sampled (samples per second) and f is the cutoff frequency you want, " << endl;
			cout << "then the parameter you would input to the program is 2*f/fs. " << endl;
			cout << "For example if your data is sampled at 5000 samples/sec and you want a bandpass filter from 500 to 1000 Hz, " << endl;
			cout << "then the lower cutoff would be 2*500/5000=0.2 and the upper cutoff would be 2*1000/5000=0.4." << endl;
	...
	if (filter_type == "lowpass")
		dcof = dcof_bwlp(filter_order, f1f);
	else if (filter_type == "highpass")
		dcof = dcof_bwhp(filter_order, f1f);
	else if (filter_type == "bandpass")
		dcof = dcof_bwbp(filter_order, f1f, f2f);
	else if (filter_type == "bandstop")
		dcof = dcof_bwbs(filter_order, f1f, f2f);

	if (dcof == NULL) {
		cout << "Unable to calculate d coefficients." << endl;
		assert(false);
	}
	...
	double* ccof = NULL;
	if (filter_type == "lowpass")
		ccof = ccof_bwlp(filter_order);
	else if (filter_type == "highpass")
		ccof = ccof_bwhp(filter_order);
	else if (filter_type == "bandpass")
		ccof = ccof_bwbp(filter_order);
	else if (filter_type == "bandstop")
		ccof = ccof_bwbs(filter_order, f1f, f2f);

	if (ccof == NULL) {
		cout << "Unable to calculate c coefficients" << endl;
		assert(false);
	}
	...
	double sf = -1.0;
	if (filter_type == "lowpass")
		sf = sf_bwlp(filter_order, f1f);
	else if (filter_type == "highpass")
		sf = sf_bwhp(filter_order, f1f);
	else if (filter_type == "bandpass")
		sf = sf_bwbp(filter_order, f1f, f2f);
	else if (filter_type == "bandstop")
		sf = sf_bwbs(filter_order, f1f, f2f);

	// ---------------------------------------------------------------------------------------------------
	// 输出coef文本
	// ---------------------------------------------------------------------------------------------------
	/* create the filter coefficient file */
	FILE *fp = fopen(filename_coef.c_str(), "w");
	if (fp == 0) {
	...
	if (filter_type == "lowpass") {
		...
		fprintf(fp, "# Butterworth highpass filter coefficients.\n");
		fprintf(fp, "# Produced by bwhp.\n");
		fprintf(fp, "# Filter order: %d\n", filter_order);
		fprintf(fp, "# Cutoff freq.: %1.15lf\n", f1f);
		fprintf(fp, "# Scaling factor: %1.15lf\n", sf);
	}
		...
		fprintf(fp, "# Butterworth bandstop filter coefficients.\n");
		fprintf(fp, "# Produced by bwbp.\n");
		fprintf(fp, "# Filter order: %d\n", filter_order);
		fprintf(fp, "# Lower cutoff freq.: %1.15lf\n", f1f);
		fprintf(fp, "# Upper cutoff freq.: %1.15lf\n", f2f);
	...
	if (filter_type == "lowpass" || filter_type == "highpass")
		upperbound = filter_order;
	else if (filter_type == "bandpass" || filter_type == "bandstop")
		upperbound = 2 * filter_order;

	fprintf(fp, "\n%d\n", upperbound + 1); /* number of c coefficients */
	if (scale_flag == 0)
	...
	fprintf(fp, "%d\n", upperbound + 1);  /* number of d coefficients */
	for (int i = 0; i <= upperbound; ++i)
		fprintf(fp, "%1.12lf\n", dcof[i]);
	...
	// 读取文本
	// ---------------------------------------------------------------------------------------------------
	setParams(filename_coef);
}

void CParamButterworthFilter::setParams(string filename_coef)
{
	// ---------------------------------------------------------------------------------------------------
	// 清空参数
	// ---------------------------------------------------------------------------------------------------
	...
	// load coef file
	// ---------------------------------------------------------------------------------------------------
	ifstream fin(filename_coef.c_str());
	string line;

	// 判断文件是否存在
	if (!fin) {
		cout << "no such file " << filename_coef << ", please check the file name!/n" << endl;
		assert(false);
	}
